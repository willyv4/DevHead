...
const CommentList = ({ viewProject, userId }: any) => {
	const [filteredProjects, setFilteredProjects] = useState<any>(
		viewProject.comments
	);

	const onSubmit = (commentId: number) => {
	...
	return (
		<div className="mt-2 border-t border-gray-200">
			<div className="mt-10">
				{filteredProjects &&
					filteredProjects.map((comment: any, idx: number) => (
						<div key={idx + comment?.author_first_name} className="m-2">
							...
							<div className="text-xs ring-1 ring-gray-200 w-11/12 m-2 p-1 pt-0 rounded">
								<br></br>
								{comment?.comment
									?.split("\n")
